...
/** JSON.parse that tolerates null/objects/strings and ignores non-JSON strings. */
...
/** Totals derived from rubric + current scores (including participation). */
...
/**
 * Single source of truth for activity lifecycle:
 * - Static: rubric, welcome, metadata
 * - Live: interim scores, participation, time reminders
 * - Results: final assessment, submission id
 * - Analytics: coarse instructor summary
 */
const initialState = {
    rubric: null,
    scoresById: {},               // { [rubric_item_id]: ScoredRubricItem }
    participationScore: null,     // { score, justification } | null
    lastActivityState: null,      // latest RTVI activity_state string
    welcomeHtml: "",
    timeReminder: null,           // human string from server
    finalAssessment: null,        // AssessmentResult
    submissionId: null,           // uuid from activity-end
    lastBlock: null,              // raw last RTVI message (for debugging/UX)
    activityMetadata: null,       // /activity/get
    analyticsSummary: null,       // /activity/summary
};
 ...
 * RTVI ingestion normalizes server messages into local shape in one place.
 */
    ...
    switch (action.type) {
        ...
        case types.SET_RUBRIC:
            return {
                ...
                // new rubric ⇒ clear transient scoring
                scoresById: {},
        ...
        /* Normalize RTVI activity messages into local actions. */
...
/**
 * Provides:
 *  - reactive state and derived totals
 *  - loaders for /activity/get and /activity/summary
 *  - a dispatch handle for advanced use (e.g., direct RTVI ingestion)
 */
export function ActivityProvider({ children }) {
    const [state, dispatch] = useReducer(reducer, initialState);

    /* Derived totals used across UI. */
    ...
    /* Rubric list joined with live scores for side panel rendering. */
    ...
    /* Loader: fetch /activity/get then seed metadata, welcome, and rubric. */
    ...
    /* Loader: fetch /activity/summary (instructor analytics snapshot). */
...
/** Read-only view of the aggregated activity state. */
...
/** Dispatch-only hook for advanced flows. */
