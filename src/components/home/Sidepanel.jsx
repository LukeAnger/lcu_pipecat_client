// src/components/home/Sidepanel.jsx
// Right-hand panel showing per-item rubric scores + optional Participation.
// Data comes from ActivityContext (rubricWithScores, participationScore, participationMax).
// When no rubric is present, shows an empty-state message.
...
/**
 * Props:
 * - setShowRight?: (bool) => void  // optional "Hide »" handler from parent split pane
 */
const Sidepanel = ({ setShowRight }) => {
    const { rubricWithScores, participationScore, participationMax } = useActivity();
                ...
                <section className="sidepanel__cards">
                    {rubricWithScores.map(({ id, title, desc, score, max }) => (
                        <article key={id} className="sidepanel__card">
                            ...
                            {/* pinned mini progress at the bottom of each card */}
